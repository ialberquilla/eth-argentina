...
1. The example hook [SwapDepositor.sol](src/SwapDepositor.sol) demonstrates the `beforeSwap()` and `afterSwap()` hooks
2. The test template [SwapDepositor.t.sol](test/SwapDepositor.t.sol) preconfigures the v4 pool manager, test tokens, and test liquidity.
